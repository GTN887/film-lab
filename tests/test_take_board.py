...
from film_lab.take_board import TakeBoardError, export_selected_to_cinema, reject_take, review_take, save_audio_cut_intent, save_take_direction, select_take, selected_take_for_shot, take_cards
...
def test_audio_cut_direction_persists_without_replacing_take(tmp_path):
    p, store, take = setup(tmp_path)
    updated = save_audio_cut_intent(p, take.id, style="J_L_CUT", j_cut_lead_s=.3, l_cut_tail_s=.4)
    intent = updated.metadata["audio_cut_intent"]
    assert updated.id == take.id and updated.media_path == take.media_path
    assert intent["j_cut"] and intent["l_cut"] and intent["j_cut_lead_s"] == .3
