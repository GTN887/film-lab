...
def _project_with_still(tmp_path):
    ...
    still = project.stills_dir / "start.png"; still.write_bytes(b"still")
...
def test_successful_generation_automatically_becomes_persistent_take(tmp_path):
    ...
    assert len(takes) == 1
    take = takes[0]
    assert take.scene_id == "scene_chase" and take.generator == "unit-render" and take.model == "unit-model"
    assert take.metadata["source"] == "generation_queue" and Path(take.media_path).is_file()
