...
VIDEO_SUFFIXES={".mp4",".webm",".mov"}; TAKE_STATES={"review","selected","rejected"}
def utc_now():
...
class ProductionStore:
 def __init__(self,project): self.project=project; project.ensure_dirs(); self.path=project.root/"production.json"
 def _load(self):
 ...
 def add_take(self,media,shot_id,scene_id="scene_001",name="",generator="",model="",prompt="",duration=None,metadata=None,copy_media=True):
  ...
  take=Take(id=take_id,scene_id=scene_id,shot_id=shot_id,media_path=str(dest.resolve()),name=name or src.stem,generator=generator,model=model,prompt=prompt,duration=duration,metadata=metadata or {}); data=self._load(); data["takes"][take.id]=asdict(take); self._save(data); return take
 ...
 def set_status(self,take_id,status):
  ...
  if status=="selected":
   for other in data["takes"].values():
    if other.get("scene_id")==raw.get("scene_id") and other.get("shot_id")==raw.get("shot_id") and other.get("id")!=take_id and other.get("status")=="selected": other["status"]="review"
