...
class QueueItem:
    ...
    def row(self) -> list[str]:
        return [
            self.shot.id,
            self.shot.name,
            self.shot.camera_move,
            f"{self.shot.duration:.1f}s",
            self.shot.intimacy_mode,
            self.status,
            self.message,
        ]
    ...
    def run(
        ...
        *,
        ...
        for index, item in enumerate(pending, start=1):
            if progress:
                progress(index, total, f"Generating {item.shot.name}")
            item.status = "running"
            item.message = f"via {generator.label}"
            try:
                output = _output_path(project, item.shot, generator.id)
                start = project.resolve_still(item.shot.start_frame)
                if start is None:
                    raise FileNotFoundError(
                        "Start frame is required. Ingest a still and assign it on the shot."
                    )
                conditioning = build_conditioning(project, item.shot, start)
                job = GenerateJob(
                    shot=item.shot,
                    start_path=start,
                    end_path=project.resolve_still(item.shot.end_frame),
                    output_path=output,
                    conditioning=conditioning,
                )
                result = generator.generate(job)
                enforcement = build_enforcement_plan(conditioning, route_metadata=generator.route_metadata() if hasattr(generator, "route_metadata") else {})
                item.output_path = str(result)
                item.status = "done"
                item.message = result.name
                ...
                ProductionStore(project).add_take(
                    ...
                    duration=duration,
                    metadata={
                        "source": "generation_queue",
                        "generation_output_path": str(Path(result).resolve()),
                        "camera_move": item.shot.camera_move,
                        "aspect_ratio": item.shot.aspect_ratio,
                        "resolution": item.shot.resolution,
                        "generator_route": generator.route_metadata() if hasattr(generator, "route_metadata") else {},
                        "conditioning": conditioning.to_dict(),
                        "generation_prompt": conditioning.prompt,
                        "continuity_enforcement": enforcement,
                    },
                )
...
def generate_one(
    ...
    if item.status != "done" or not item.output_path:
        raise RuntimeError(item.message or "Generation failed")
